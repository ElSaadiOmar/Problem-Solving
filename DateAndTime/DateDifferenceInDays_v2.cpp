// Problem: Calculate the difference in days between two dates, in either order.
// Automatically swaps the dates if they were entered in reverse order.
...
int GetDifferenceInDays(sDate Date1, sDate Date2, bool IncludingEndDay = false){
    ...
    if(IsDate1BeforeDate2(Date2,Date1))
        return -1*GetDifferenceInDays(Date2,Date1,IncludingEndDay);
