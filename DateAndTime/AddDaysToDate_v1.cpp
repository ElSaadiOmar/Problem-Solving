// Problem: Add a number of days to a date, handling month/year rollover.
// First attempt at the day-addition logic (see v2 for a refined version).
...
int NumOfYear(int Number, short &Year)
{
    while (true)
    {
        if (IsLeapYear(Year))
...
sDate DateAddDays(int DaysAdd, sDate Date)
{
    ...
    int Number = NumOfYear(TotalNumber, Date.Year);
    sDate Date = CalculateTheDateOfNum(Number, Date.Year);
    return Date;
}
int main()
{
