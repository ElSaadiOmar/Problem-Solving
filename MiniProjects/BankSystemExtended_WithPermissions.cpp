// Mini Project: Extended bank system with client/user management (full solution).
// Adds CheckAccessPermission() so each menu action respects the logged-in user's access level.
...
bool CheckAccessPermission(enMainMenuPermission Access);
...
{
    if (!CheckAccessPermission(enMainMenuPermission::pListClients))
    {
...
void ShowUpdateClientScreen()
{
    ...
    if (!CheckAccessPermission(enMainMenuPermission::pUpdateClients))
    {
        ShowAccessDeniedMessage();
        return;
    }
...
void ShowTransactionMenuScreen()
{
    ...
    if (!CheckAccessPermission(enMainMenuPermission::pTransactions))
    {
        ShowAccessDeniedMessage();
        GoBackToMainMenu();
    }
...
bool CheckAccessPermission(enMainMenuPermission Access)
{
    return (CurrentUser.Access == FULLAccess) || ((CurrentUser.Access & Access) == Access);
}
...
bool FindUserByUserNameAndPassword(string UserName, string Password, sUser &User)
{
    ...
    if (MyFile.is_open())
    {
        ...
        {
            UserOfFile = ConvertLinetoRecordOfUser(Line);
            if (UserOfFile.UserName == UserName && UserOfFile.Password == Password)
            {
                User = UserOfFile;
                MyFile.close();
                return true;
